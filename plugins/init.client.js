...
import { AbsFileSystem } from '@/plugins/webos'
...
Vue.prototype.$sanitizeFilename = (input, colonReplacement = ' - ') => {
  ...
  if (sanitized.length > MAX_FILENAME_LEN) {
    ...
    var ext = sanitized.includes('.') ? sanitized.substring(sanitized.lastIndexOf('.')) : ''
    var basename = ext ? sanitized.substring(0, sanitized.lastIndexOf('.')) : sanitized
